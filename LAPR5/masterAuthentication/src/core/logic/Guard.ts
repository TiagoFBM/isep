...
export class Guard {
  ...
  public static followsRegex(string: string, regex: RegExp, argumentName: string): IGuardResult {
    if (regex.test(string)) {
      return { succeeded: true };
    } else {
      return {succeeded: false, message: `${argumentName} does not follow the correct pattern.`};
    }
  }
