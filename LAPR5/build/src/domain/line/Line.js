...
class Line extends AggregateRoot_1.AggregateRoot {
    ...
    addAllowedVehicleTypes(vehicleType) {
        const newLength = this.props.allowedVehicles.length + 1;
        if (this.props.allowedVehicles.push(vehicleType) == newLength) {
            return true;
        }
        else {
            return false;
    ...
    static create(LineDTO) {
        ...
        LineDTO.linePaths.forEach(element => {
          var linePathOrFailure = LinePath.create(element as ILinePathDTO);
          if (LinePath.create(element)) newLinePaths.push(linePathOrFailure.getValue());
        });
        */
        var newLinePaths = [];
