...
public class Graph<V,E> implements GraphInterface<V,E> {
    ...
    public Iterable<Edge<V, E>> edges() {
        List<Edge<V, E>> listaEdges = new LinkedList<>();
        Iterator<Vertex<V, E>> it = vertices.values().iterator();
        while (it.hasNext()) {
            Vertex<V, E> vert = it.next();
            Iterator<Edge<V, E>> it2 = vert.getAllOutEdges().iterator();
            while (it2.hasNext()) {
                listaEdges.add(it2.next());
            }
        }
        return listaEdges;
    }
    ...
    public Iterable<Edge<V, E>> outgoingEdgesSortedByWeight(V vert) {
        if (!validVertex(vert))
            return null;

        Vertex<V, E> vertex = vertices.get(vert);

        Iterable<Edge<V, E>> listOutEdges =  vertex.getAllOutEdges();
        List<Edge<V, E>> novaLista = new LinkedList<>();
        for (Edge<V, E> edge : listOutEdges) {
            novaLista.add(edge);
        }
        for (int i = 0; i < novaLista.size() - 1; i++) {
            for (int j = i; j < novaLista.size(); j++) {
                if (novaLista.get(i).getWeight() > novaLista.get(j).getWeight()) {
                    Edge<V, E> temp = novaLista.get(i);
                    novaLista.set(i, novaLista.get(j));
                    novaLista.set(j, temp);
                }
            }
        }
        return novaLista;
    ...
    public Iterable<Edge<V, E>> incomingEdges(V vert) {
        List<Edge<V, E>> lista = new LinkedList<>();
        Iterator<Edge<V, E>> it = edges().iterator();
        while (it.hasNext()) {
            Edge<V, E> edg = it.next();
            if (edg.getVDest().equals(vert)) {
                lista.add(edg);
            }
        }
        return lista;
